...
ALTER TABLE `securich`.`sec_us_ho_db_tb_ro` ADD COLUMN `STATE` CHAR(1) NULL DEFAULT 'A'  AFTER `RO_ID` ;

ALTER TABLE `securich`.`sec_us_ho_db_sp_ro` ADD COLUMN `STATE` CHAR(1) NULL DEFAULT 'A'  AFTER `RO_ID` ;

INSERT INTO `securich`.`sec_reserved_usernames` VALUES
   (1,'root'),
   (2,''),
   (3,'msandbox');
...
INSERT  INTO `securich`.`sec_help`(`ID`,`STOREDPROCEDURE`,`DESCRIPTION`) VALUES (4,'check_full_user_entries','check_full_user_entries(\'username\'); (version 0.1.1)\r\n-- Checks the roles assigned to a particular user and on which database, table and from which host those privileges can be used');
INSERT  INTO `securich`.`sec_help`(`ID`,`STOREDPROCEDURE`,`DESCRIPTION`) VALUES (5,'check_privilege_users','check_privilege_users(\'privilege\'); (version 0.1.1)\r\n-- Shows a list of users who have been granted a particular privilege\r\n    check_privilege_users(\'privilege\',\'databasename\',\'tablename\') (version 0.1.2);\r\n-- Shows a list of users who have been granted a particular privilege on a particular table in a particular database.');
INSERT  INTO `securich`.`sec_help`(`ID`,`STOREDPROCEDURE`,`DESCRIPTION`) VALUES (6,'check_role_privileges','check_role_privileges(\'rolename\'); (version 0.1.1)\r\n-- Shows a list of privileges belonging to a particular role');
INSERT  INTO `securich`.`sec_help`(`ID`,`STOREDPROCEDURE`,`DESCRIPTION`) VALUES (7,'check_roles','check_roles(); (version 0.1.1)\r\n-- Run the above in order to check which roles are available');
INSERT  INTO `securich`.`sec_help`(`ID`,`STOREDPROCEDURE`,`DESCRIPTION`) VALUES (8,'check_user_entries','check_user_entries(\'username\'); (version 0.1.1)\r\n-- Checks the roles assigned to a particular user and on which database and from which host those privileges can be used');
INSERT  INTO `securich`.`sec_help`(`ID`,`STOREDPROCEDURE`,`DESCRIPTION`) VALUES (9,'check_user_list','check_user_list(); (version 0.1.1)\r\n-- Run the above in order to obtain a list of user@host present in the system');
INSERT  INTO `securich`.`sec_help`(`ID`,`STOREDPROCEDURE`,`DESCRIPTION`) VALUES (10,'check_user_privileges','check_user_privileges(\'username\',\'hostname\',\'databasename\',\'rolename\'); (version 0.1.1)\r\n-- Lets the administrator check the privileges a user has on that database or place \'all\' instead of \'rolename\' in order to have a look at all the privileges on that particular combination.');
