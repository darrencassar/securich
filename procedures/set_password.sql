  ...
  BEGIN
    ...
    SET ROOTUSER= (select 'root'=(substring_index(user(),'@',1)));
     ...
     IF CORRECTUSER = 1 OR ROOTUSER = 1 THEN
        ...
        ELSEIF ((select newpasswordin REGEXP "[[[:alpha:]+][[:digit:]+][[:punct:]+]|[[:alpha:]+][[:punct:]+][[:digit:]+]|[[:punct:]+][[:alpha:]+][[:digit:]+]|[[:punct:]+][[:digit:]+][[:alpha:]+]|[[:digit:]+][[:alpha:]+][[:punct:]+]|[[:digit:]+][[:punct:]+][[:alpha:]+]]") = 0 OR (select length(newpasswordin)) < 10 ) and ROOTUSER <> 1 /*newpasswordin = ''*/ THEN

           select "Invalid password - Password must be at least 8 characters long and include at least a number, a character and one of the following   !\"$%^&*()-_=+[]{}\'@;:#~,.<>/\?|";
